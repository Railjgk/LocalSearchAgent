...
def test_intent_parser_extracts_karaoke_activity_keyword() -> None:
    raw_text = "今晚和朋友吃火锅，5个人，预算600，吃完火锅去唱歌"
    intent = parse_intent(raw_text)
    constraints = constraints_from_intent(intent)

    assert constraints["scene"] == "friends"
    assert constraints["people_count"] == 5
    assert "KTV欢唱" in constraints["planning_preferences"]["activity_type"]
    assert "火锅" in constraints["planning_preferences"]["restaurant_type"]


def test_explicit_activity_type_missing_supply_blocks_fallback() -> None:
    raw_text = "今晚和朋友吃火锅，5个人，预算600，吃完火锅去唱歌"
    intent = parse_intent(raw_text)
    constraints = constraints_from_intent(intent)
    state = {
    ...
    candidate_out = candidate_generator_node(state)
    state.update(candidate_out)
    filter_out = constraint_filter_node(state)

    assert candidate_out["candidates"] == []
    assert candidate_out["candidate_generation_issues"][0]["type"] == "missing_activity_supply"
    assert "显式活动或餐饮类型" in filter_out["filter_reasons"]["_summary"]


def test_intent_parser_extracts_sports_and_hotpot_keywords() -> None:
    ...
    assert constraints["scene"] == "friends"
    assert constraints["people_count"] == 5
    assert "运动" in constraints["planning_preferences"]["activity_type"]
    assert "火锅" in constraints["planning_preferences"]["restaurant_type"]
    assert constraints["sequence_preference"] == "restaurant_then_activity"


def test_explicit_activity_and_restaurant_types_are_hard_filtered() -> None:
    raw_text = "今晚和朋友吃火锅，5个人，预算600，吃完火锅去运动"
    intent = parse_intent(raw_text)
    constraints = constraints_from_intent(intent)
    state = {
    ...
    assert candidate_out["candidates"]
    for plan in candidate_out["candidates"]:
        ...
        assert activity.get("category") == "sports"
